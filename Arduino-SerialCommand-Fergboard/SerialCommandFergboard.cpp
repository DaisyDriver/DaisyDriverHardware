...
//
// Add to buffer from another source
//
void SerialCommand::addBuffer(char nonserial)
{
	inChar = nonserial;
	interpret();
}
...
void SerialCommand::addBufferMod(char *nonserial)
{
	char inputstring[SERIALCOMMANDBUFFER];
	int i = 0;
	strcpy(inputstring, nonserial);
	inChar = inputstring[i];
...
// This function interprets the buffer, and executes commands as they come in.
// When the terminator character (default '\r') is seen, it starts parsing the 
// buffer for a prefix command, and calls handlers setup by addCommand() member
void SerialCommand::interpret()
{
int i; 
boolean matched; 
