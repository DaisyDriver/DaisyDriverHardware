...
class SerialCommand
{
	public:
		...
		void addBuffer(char nonserial); // capture text from any source and add it to the command buffer
		void addBufferMod(char *);
		void clearBuffer();   // Sets the command buffer to all '\0' (nulls)
		void interpret();     // interpret the command buffer
		char *next();         // returns pointer to next token found in command buffer (for getting arguments to commands)
		void readSerial();    // Capture serial and add it to the command buffer  
